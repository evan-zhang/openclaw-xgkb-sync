 ...
 */
function detectLocalRenames(localFiles, dbRecords, folderPathToRemoteId) {
    // 构建 inode → 当前本地条目 的映射
 ...
 */
function detectDirectoryMoves(dbRecords, inodeToEntry, folderPathToRemoteId) {
    const plans = [];
    const consumedFromPaths = new Set();
    const consumedToPaths = new Set();
    // 按直接父目录分组（只取有 inode 信息的记录）
    const dirToDirectChildren = new Map();
    for (const rec of dbRecords) {
        if (!rec.remoteFileId || !rec.localDev || !rec.localIno || rec.localIno === 0)
            continue;
        const dir = dirOf(rec.localPath);
        if (!dir)
            continue; // 根目录直接子文件不做目录级合并
    ...
    for (const [oldDir, directChildren] of dirToDirectChildren) {
        ...
        if (!valid || newDir === null || newDir === oldDir || !newDir)
            continue;
        // 目标父目录须可解析
        if (!folderPathToRemoteId.has(dirOf(newDir)))
            continue;
        // 获取本目录的远端 folderId（从任意直接子文件的 remoteFolderId）
        const folderIdRec = directChildren.find((r) => r.remoteFolderId);
        if (!folderIdRec?.remoteFolderId)
            continue;
